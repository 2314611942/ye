...
function App() {
  ...
  const epilogueEntry = useRef(null);
  const sound = useSound(Boolean(panel), epilogue);
  const openEpilogue = () => { setPlaying(false); setPanel(null); setEpilogue(true); setLink(null); };
  const closeEpilogue = () => { setEpilogue(false); requestAnimationFrame(() => epilogueEntry.current?.focus()); };
  ...
  useEffect(() => {
    if (!playing || panel || !entered || epilogue) return;
    const timer = setTimeout(() => { const next = allEvents[allEvents.findIndex((e) => e.id === selectedId) + 1]; if (next) { select(next.id, false); setFocusRequest((n) => n + 1); } else { openEpilogue(); } }, 10000);
    return () => clearTimeout(timer);
  }, [playing, panel, selectedId, entered, epilogue]);
  useEffect(() => {
  ...
  return <div className={`exhibit ${panel ? 'reading' : ''} ${entered ? 'entered' : ''} mode-${mode}`}>
    ...
    <div className="exhibit-space" inert={!entered || epilogue ? true : undefined}>
      <header className="exhibit-header"><div className="brand"><RedStar/><div><strong>叶挺<span> · 生平足迹</span></strong></div></div>
      ...
      <main id="exhibition-map" className="exhibition-map" role="tabpanel" aria-labelledby={`tab-${period.id}`}>
        <MapAtlas period={period} selected={selected} onSelect={(id) => select(id)} mode={mode} setMode={(m) => { setPlaying(false); setFocusRequest(0); setMode(m); }} showRoutes={showRoutes} focusRequest={focusRequest} reducedMotion={reducedMotion} onInteract={() => setPlaying(false)} paused={Boolean(panel) || epilogue}/>
        <RedRibbons className="map-ribbons"/><YananSkyline className="map-skyline"/><Particles paused={Boolean(panel) || epilogue || !entered || reducedMotion}/>
        ...
        <button ref={epilogueEntry} className="ending-entry" onClick={openEpilogue} aria-label="观看结语" aria-haspopup="dialog">
          <span className="ending-entry-play" aria-hidden="true"><Play size={18} fill="currentColor"/></span>
          <span className="ending-entry-copy"><small>山河新篇</small><strong>观看结语</strong></span>
          <ArrowUpRight className="ending-entry-arrow" size={18} aria-hidden="true"/>
        </button>
      </main>
    ...
    {!entered && <Entrance onEnter={enter} reducedMotion={reducedMotion}/>}
    <ExhibitDialog panel={panel} event={selected} onClose={close} onStep={step} onEpilogue={openEpilogue} onOpenEvent={(id) => select(id)} sound={sound} reducedMotion={reducedMotion} setReducedMotion={setReducedMotion} showRoutes={showRoutes} setShowRoutes={setShowRoutes} onShare={share} shareUrl={shareUrl.href}/>
    {epilogue && <Epilogue onClose={closeEpilogue} initialVolume={sound.volume} reducedMotion={reducedMotion}/>}
